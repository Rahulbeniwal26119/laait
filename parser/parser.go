...
type Parser struct {
	...
	// tracker of operators and methods associated with them
...
func New(l *lexer.Lexer) *Parser {
	...
	// register the function to token
	...
	p.registerPrefix(token.LTBRACKET, p.parseArrayLiteral)
	p.registerPrefix(token.LTBRACE, p.parseHashLiteral)
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RTBRACE) {
		p.nextToken()
		...
		if !p.peekTokenIs(token.RTBRACE) && !p.expectedPeek(token.COMMA) {
			return nil
		}
	}
	if !p.expectedPeek(token.RTBRACE) {
		return nil
	}
...
func (p *Parser) parseArrayLiteral() ast.Expression {
	...
	array.Elements = p.parseExpressionList(token.RTBRACKET)
	return array
}
...
func (p *Parser) parseCallExpression(function ast.Expression) ast.Expression {
	...
	exp.Arguments = p.parseExpressionList(token.RTPAREN)
	return exp
}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	expression.Effect = p.parseBlockStatement()
	...
	if p.peekTokenIs(token.ELSE) {
		...
		expression.Optional = p.parseBlockStatement()
	}
	return expression
}
...
func (p *Parser) parseBlockStatement() *ast.BlockStatement {
	...
	for !p.curTokenIs(token.RTBRACE) && !p.curTokenIs(token.FILEEND) {
...
func (p *Parser) peekError(t token.TokenType) {
	msg := fmt.Sprintf("excepted next token to be %s, got %s", t, p.peekToken.Type)
